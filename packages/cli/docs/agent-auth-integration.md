...
Before registration, tell the person that Inth will receive their email address and the requested permissions. Start only after they agree. `login --email` and `signup --email` treat the command itself as confirmation to send those details, so they do not require `--yes`. The lower-level `auth start` command requires `--yes` to confirm the same action. These commands only request access; the person must still approve the permissions in their browser. For hosted c15t setup:
...
Run this command in a background terminal as soon as you have shown the link. It waits at the server's polling interval and returns one final `authenticated` result after approval, selecting this connection for later commands. The person does not need to tell the agent that they finished. The default timeout is 600 seconds; `--timeout` accepts 1 to 3600 seconds. Timeout or cancellation between polls, during discovery, or while waiting for the credential lock preserves the pending claim, so the same command can resume in another process. Interrupting a single-use claim exchange can instead return `claim_uncertain`. Disconnect with `inth logout --auth agent --json` and start a new claim in that case. Without `--wait`, completion still checks once. The claim token, access token and identity assertion are stored in the OS credential store and never printed.
...
inth project create --organization <organization-id> \
  ...
  --trusted-origins '["example.com"]' --json
```
