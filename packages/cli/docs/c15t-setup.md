...
For an auth.md credential, first complete [agent sign-in](agent-auth-integration.md) with `organizations.read,organizations.write,projects.read,projects.write`, then add `--auth agent` to every API command below. This requires the server's CLI rollout. Human account verification and permission approval happen in the browser.
