...
# being signed, runs while the signing key and notary credentials are loaded.
set -eu
...
: "${TEAM_ID:?Set TEAM_ID}"
: "${NOTARY_KEY:?Set NOTARY_KEY to the base64 App Store Connect API key}"
...
/usr/bin/xcrun notarytool submit "$work/inth.zip" --key "$work/notary.p8" \
  ...
  --wait --timeout 30m --output-format json > "$work/notary.json" || true
# Check the status instead of the exit code so a rejection prints Apple's log.
