...
Sign-in and API commands need the same approval. They write locks and settings to the [state directory](./authentication#where-does-the-inth-cli-store-credentials) and connect to `api.inth.com`. Adding `api.inth.com` to Cursor's network allowlist does not help, because the CLI does not use the sandbox's HTTP proxy. When Cursor's sandbox blocks either one, the CLI fails with the `sandbox_restricted` error code. `inth --help` and `inth --version` work inside the sandbox.
