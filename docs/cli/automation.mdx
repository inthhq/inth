...
For hosted consent setup, follow [Set up hosted c15t with an agent](https://github.com/inthhq/inth/blob/main/packages/cli/docs/c15t-setup.md). Existing browser sign-in supports organization and project creation from JSON commands. For a separate, explicitly scoped credential, use [auth.md sign-in](https://github.com/inthhq/inth/blob/main/packages/cli/docs/agent-auth-integration.md).

Use `--json` for machine-readable output. Named commands default to formatted text, including when piped or run with `--non-interactive`. The raw `api` command prints JSON by default; add `--json` for the standard CLI envelope. Discover commands and flags with `inth --help --json`; read the CLI version with `inth --version --json`.
...
The default wait is ten minutes; use `--timeout <seconds>` to change it. A timeout or cancellation between polls, during discovery, or while waiting for the credential lock preserves the pending sign-in. Run the waiting command again to resume.
...
The envelope is defined in [output.schema.json](https://github.com/inthhq/inth/blob/main/packages/cli/output.schema.json). Consumers should branch on `ok` and `error.code`, rather than matching error messages or relying on JSON property order. `error.apiCode` preserves recognized server codes such as `INSUFFICIENT_CREDITS`, `CONFLICT`, and `PLAN_LIMIT_REACHED`; it is null for local errors or unrecognized server codes.
