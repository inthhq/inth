...
When Cursor's sandbox blocks the CLI state directory or the Inth API, the CLI now returns the `sandbox_restricted` error code. The message says what was blocked and asks you to run the command outside the sandbox. The install docs also explain how to install from a sandboxed agent terminal, including npm's misleading report of a root-owned cache.
